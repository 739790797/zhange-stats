...
export function MihoyoAttendanceCalendarButton({
  ...
  const queryKey = ["mihoyo-attendance-calendar", gameCode, roleUid];

  const query = useQuery({
    queryKey,
    queryFn: () => fetchMihoyoAttendanceCalendar(gameCode, roleUid, false),
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    try {
      const data = await fetchMihoyoAttendanceCalendar(gameCode, roleUid, true);
      ...
      >
        {query.isLoading ? (
          ...
          <Space direction="vertical" size={12} style={{ width: "100%" }}>
            ...
            {query.data.progress_reliable === false ? (
              <Alert
                type="info"
                showIcon
                message="本月有漏签，格子按已签次数顺序标记，可能与官方日历日期不完全对应。"
              />
            ) : null}
            <Space wrap size={8}>
              <Tag color="blue">
