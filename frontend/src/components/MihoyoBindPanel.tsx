...
import type { MihoyoQrStart, MihoyoStatus } from "@/api/types";
...
export function MihoyoBindPanel({
  ...
  const [qr, setQr] = useState<MihoyoQrStart | null>(null);
  const [qrHint, setQrHint] = useState("请使用米游社 App 扫码");
  ...
  const QR_HINT_WAITING = "请使用米游社 App 扫码，并在手机上确认登录";
  ...
  const startQrSession = async () => {
    ...
    try {
      const session = await startMihoyoQrBind();
      setQr(session);
      setQrHint(QR_HINT_WAITING);
      pollTimer.current = window.setInterval(async () => {
        if (qrDone.current || modeRef.current !== "qr") return;
        try {
          const poll = await pollMihoyoQrBind(session.scan_id);
          ...
          if (poll.status === "ok") {
            qrDone.current = true;
            clearPollTimer();
            await finishOk(null, poll.message || "米游社绑定成功");
            return;
          }
