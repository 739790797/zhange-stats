...
export const LEGAL_DOCS: Record<LegalDocId, LegalDoc> = {
  terms: {
    ...
    paragraphs: [
      ...
      "联机房间供队友协作勾选任务、标点与声明钥匙，不提供透视、雷达或任何修改游戏的能力。截图同步只解析你本机游戏截图文件名中的坐标，并仅广播给同一房间的入座成员（含你在其他设备上打开的同一账号）。",
  ...
  privacy: {
    ...
    updated: "2026-09-08",
    paragraphs: [
      "我们收集账号注册所用的邮箱或 QQ 互联标识、显示名，以及你主动填写的资料。登录态保存在 HttpOnly Cookie 中，浏览器脚本读不到 access token。",
      "联机房间会保存房间标题、地图、成员显示名、任务认领、钥匙声明、地图记号与简要在线状态。公开大厅可看到公开房的名称、地图、人数和在座昵称。私密房需密码才能入座；未入座者看不到棋盘、钥匙与人员名单。",
      "战局日志在你的浏览器本机解析。服务器只保存地图、战局编号、时间等摘要，不保存游戏日志原文。局内坐标只在房间在线成员之间内存广播，不落库。钥匙管理的截图识别会把你粘贴的游戏截图发到本站，在内存中读短名后立即丢弃，不落盘、不入库。",
      "若你绑定森空岛等平台签到，相关凭证加密存库，仅用于你发起的签到或盒子查询。宣传塔科夫功能时，不要求绑定这些平台。",
      "日志与访问记录用于排障和防滥用。我们不会把个人数据出售给广告商。",
      "可在个人中心申请注销账号（邮箱验证码确认）。我们会解绑平台凭证、删除头像与草稿；已发布的酒馆文章会保留，作者显示为「已注销用户」。申请通常即时完成，最迟 15 日内处理。继续使用即视为了解本说明。",
    ],
  },
};
