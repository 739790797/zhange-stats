"""Add taygedo binds and checkin logs.
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["bind_id"], ["taygedo_binds.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["member_id"], ["members.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "member_id",
    ...
    op.create_index(
        op.f("ix_taygedo_checkin_logs_member_id"),
        "taygedo_checkin_logs",
        ["member_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_taygedo_checkin_logs_bind_id"),
        "taygedo_checkin_logs",
        ["bind_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_taygedo_checkin_logs_checkin_date"),
        "taygedo_checkin_logs",
        ["checkin_date"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index(op.f("ix_taygedo_checkin_logs_checkin_date"), table_name="taygedo_checkin_logs")
    op.drop_index(op.f("ix_taygedo_checkin_logs_bind_id"), table_name="taygedo_checkin_logs")
    op.drop_index(op.f("ix_taygedo_checkin_logs_member_id"), table_name="taygedo_checkin_logs")
    op.drop_table("taygedo_checkin_logs")
    op.drop_table("taygedo_binds")
