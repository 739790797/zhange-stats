"""Add kujiequ_attendance_raws for game sign-in calendar cache.
...
down_revision: Union[str, Sequence[str], None] = "20260807_0030"
...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_kujiequ_attendance_raws_member_id"),
        "kujiequ_attendance_raws",
        ["member_id"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index(
        op.f("ix_kujiequ_attendance_raws_member_id"),
        table_name="kujiequ_attendance_raws",
    )
    op.drop_table("kujiequ_attendance_raws")
