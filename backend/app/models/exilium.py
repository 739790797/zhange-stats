...
class ExiliumBind(Base):
    """追放社区账号绑定（凭证 JSON 加密存储）。"""

    __tablename__ = "exilium_binds"
    ...
    member = relationship("Member", back_populates="exilium_bind")
    logs = relationship(
        "ExiliumCheckinLog",
        back_populates="bind",
...
class ExiliumCheckinLog(Base):
    ...
    bind = relationship("ExiliumBind", back_populates="logs")
