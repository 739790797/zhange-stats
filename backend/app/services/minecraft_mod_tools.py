...
from app.services.minecraft_rcon import MinecraftRconError, rcon_exec
...
def _infer_runtime_loader(blob: str) -> str:
    from app.services.minecraft_eggs import infer_loader
...
def _live_egg_context(db: Session) -> dict[str, str]:
    ...
    mc_version = ""
    try:
        from app.services.minecraft_eggs import inspect_current_egg
...
def _invalidate_scans() -> None:
    ...
    try:
        from app.services.minecraft_profile import invalidate_live_public_facts
...
def _preset_blob(db: Session) -> dict[str, Any]:
    from app.services.minecraft_profile import get_or_create_profile
