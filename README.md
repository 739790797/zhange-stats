...
开源的圈子成员站：把多平台签到与养成盒、Steam 游玩统计，以及塔科夫图鉴、Minecraft 单服代操收在同一处。公开的 **战鸽酒馆** 可以读文章，登录后可以评论。**逃离塔科夫** 图鉴未登录可读，个人进度与开房须登录。
...
- **Steam**：绑定后记录游戏中会话；按日看时间轴，按周 / 月 / 年看热力日历。时间轴上的昵称链到 Steam 社区资料。圈子成员可互看。
- **战鸽助手**：Windows 桌面端用 WebView 打开本站。嵌入时不画站点侧栏；`/app` 是助手首页，打开时先确认登录。塔科夫截图和日志的读盘交给助手，解析仍在站点。详见 [`docs/security.md`](docs/security.md)「战鸽助手」。
- **我的日常**：已加入本站的各平台签到任务与今日奖励。
- **战鸽酒馆**：公开文章站。未登录可读；登录后可评论。
- **签到平台**：森空岛、塔吉多、库街区、米游社、追放。绑定后可手动签到或按角色定时自动签到；部分平台支持兑换与养成盒。
- **逃离塔科夫**：未登录可查物品、任务、商人、BOSS、地图、藏身处与钥匙分类百科、工作台计算与联机大厅预览；登录后管理个人进度、钥匙拥有、藏身处规划与任务线互斥，并开/加入联机房间。钥匙箱 / 局前准备截图识别走服务端。游戏日志在浏览器本机解析并可审阅同步结果，服务器只存摘要。离线战局可手选地图挂截图坐标。
- **Minecraft**：圈子单服代操（服况、启停、文件与模组工具）。

管理员可配置用户、任务、调度、集成密钥、文字识别、文件占用与运行环境。
