#!/usr/bin/env bash
# 战鸽数据备份：MySQL + var/data + var/uploads + .env
# 用法（安装根）：sudo ./scripts/backup.sh
# 覆盖目录：ZHANGE_BACKUP_DIR=/var/backups/zhange
# 保留天数：ZHANGE_BACKUP_KEEP_DAYS=14
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ENV_FILE="${REPO_ROOT}/.env"
BACKUP_ROOT="${ZHANGE_BACKUP_DIR:-/var/backups/zhange}"
