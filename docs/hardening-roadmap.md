...
| 7 | CSP：先 Report-Only 再收紧 | L | 1 字体自托管 | **已落地**（Report-Only；enforce 由运营打开） |
...
**原目标：** 系统更新、删除用户、保存集成密钥，不能只靠已登录的管理员 JWT。做法曾是邮箱验证码（不引入 TOTP）。

**落地拍板：** 管理端保存**不再**发步进码。管理员会话被盗即等于能改配置；继续靠 HttpOnly Cookie、CSRF、生产 `Secure`、短 JWT。用户侧注册 / 绑邮 / 找回 / 注销仍走邮箱验证码。`ALLOW_EMAIL_CODE_LOG` 生产仍禁止。
...
**已于 2026-09-08 全部落地。** 本文件是归档验收清单，不是待办。

生产侧（运维，不阻塞本路线关闭）：

- CSP 默认 Report-Only；观察 `zhange.csp` 无误杀联机 WS / 地图 / KaTeX / 极验后，可设 `CSP_ENFORCE=true`（见 [`deploy.md`](deploy.md)）
- 备份恢复演练日期记在运维笔记（见公开运营检查）
