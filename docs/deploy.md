...
单副本源码部署。本机没有可用库时，`scripts/linux/install.sh` 会自动安装 **MariaDB**（Debian/Ubuntu `apt`）、建库并写回 `DATABASE_URL`；已有 127.0.0.1 上的库则只建库/用户。跳过：`ZHANGE_SKIP_MARIADB=1`。塔科夫物品 raw 约 20MB，建议 `max_allowed_packet ≥ 64M`（脚本会写入）。可选本机 Redis（限流 / 扫码 KV；不配则进程内降级）。

```bash
git clone https://github.com/739790797/zhange-stats.git /opt/zhange-stats
cd /opt/zhange-stats
# 本机 .env 自行设 APP_ENV=production（脚本不会写入）
...
已有远程库：先把 `.env` 里 `DATABASE_URL` 改成真实连接串（非 `root:password@127.0.0.1` 占位），再跑 `scripts/linux/install.sh`（不会覆盖手写 URL）。

## Windows 本机部署

公开生产仍推荐 Linux LXC（管理端一键更新、systemd）。Windows 用于开发机或单机自托管：便携 MariaDB 11.4 落到 `var/mariadb/`（无需管理员 / winget），数据目录与 `provision.json` 不可从文件管理下载。
...
脚本不区分生产/开发、也没有 `-Production`。要当生产跑，在**这一份安装树**的 `.env` 里设 `APP_ENV=production`。跳过装库：`$env:ZHANGE_SKIP_MARIADB=1`。本机 3306 已被占用且占位账号连不上时，便携实例改用 3307。`install` / `run` / `restart` 都会检查依赖（已就绪则跳过 pip）。
...
应急排障（非常规升级路径）：`sudo bash scripts/linux/restart.sh`。  
下载被中断、或迁移半完成 / 历史 **Alembic 双 0056** 导致进程起不来、无法用管理端更新时，在主机安装树内：

```bash
git pull
sudo bash scripts/linux/install.sh
sudo bash scripts/linux/restart.sh
```

不要用 curl 管道拉外部脚本覆盖本机。正式安装只吃上一成功 GitHub Release；`git pull` 主干可能含尚未过门的提交。
...
持久化目录：`var/data/`（含 `.secret_key` 与日志、TexTeller 公式识别权重 `var/data/texteller/`、钥匙截图识别 RapidOCR 权重 `var/data/rapidocr/` 与 EasyOCR 权重 `var/data/easyocr/`）、`var/uploads/`、`.env`（更新白名单不会覆盖）。管理端「文件管理」可查看这些目录及 venv / 缓存占用。已有生产若仍用安装根 `data/`、`uploads/`，保持 `.env` 原值即可。相对路径相对安装根，不要往 `backend/`、`frontend/` 写 data/uploads。
...
钥匙管理与局前任务页截图识别走站内共享 OCR：[RapidOCR](https://github.com/RapidAI/RapidOCR)（默认 **PP-OCRv5 server**，站点设置「文字识别」从本机 RapidOCR onnx 清单选档位）和 [EasyOCR](https://github.com/JaidedAI/EasyOCR)（钥匙箱默认交叉验证；局前任务默认只开熊猫）。低频场景优先准。依赖在 `requirements.txt`（`rapidocr` + `easyocr` / **CPU** torch + `opencv-python-headless`）。安装与一键更新会先装官方 CPU 轮，避免 EasyOCR 从 PyPI 拉数 GB CUDA。权重由「文字识别」页检查更新或任务配置「识别模型更新」落到 `var/data/rapidocr` 与 `var/data/easyocr`（Paddle 走 ModelScope，EasyOCR 走 GitHub Release）；识别时不现场下载，未就绪会 503。EasyOCR 常驻大约多占 1–2GB 内存。LXC 若同时装上了 `opencv-python`（带 GUI）可能缺 libGL，可 `pip uninstall -y opencv-python` 只留 headless。不对外提供通用识别 HTTP。

健康检查：`GET /health` 返回 `status` / `database` / `scheduler` / `version`；数据库不通时为 `degraded` 且 **HTTP 503**。数据库探测结果进程内缓存 1 秒，避免探针打满连接池。管理端「运行维护 → 运行状态」另含 Redis、SMTP，以及 `APP_ENV` / 访客 IP（`TRUST_X_FORWARDED_FOR`）说明；公开引流前应在该页核对。
...
| 管理端核对 | 「运行维护 → 运行状态」：MySQL / Redis / 调度器 / SMTP；运行环境与访客 IP 为说明而非服务卡片。生产未配 Redis 或 SMTP 会标降级 |
...
- 备份：`sudo ./scripts/linux/backup.sh`（读 `.env` 的 `DATABASE_URL`，默认写到本安装树 `var/backups/zhange-时间戳.tar.gz`，保留 14 天）。可用 `ZHANGE_BACKUP_DIR`、`ZHANGE_BACKUP_KEEP_DAYS` 覆盖。Windows：`scripts\win\backup.ps1`。
- 恢复：`sudo ZHANGE_RESTORE_CONFIRM=YES ZHANGE_RESTORE_ARCHIVE=/opt/zhange-stats/var/backups/zhange-时间戳.tar.gz ./scripts/linux/restore.sh`（停应用 → 导库 → 解压 `var/` 与 `.env` → 启动）。Windows 同样设置这两个环境变量后执行 `scripts\win\restore.ps1`。
