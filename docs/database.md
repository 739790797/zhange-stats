...
tarkov_map_places
tarkov_user_key_owns · tarkov_user_task_dones · tarkov_user_task_starteds · tarkov_user_raid_logs · tarkov_user_raid_preps
...
| `tarkov_user_task_dones` | 用户任务完成勾选（按 `game_mode`=`pvp`/`pve` 分开）；复合主键 `(user_id, game_mode, task_id)`。个人中心任务树「已完成」，与 `tarkov_user_task_starteds` 组成账号进度账。同一任务若同时出现在两边，完成优先。ON DELETE CASCADE |
